...
Queue *initQueue()
{
    ...
    if(newQueue->size == NULL || newQueue->size == NULL || newQueue->dummy ==NULL)
    {
...
int enQueue(Queue *queueToAdd, void *obj)
{
    //Increment size
...
{
    //Get the object we want via peek, check that peek was successful then free the node object and mend the list
    void *obj = peek(queueToRemove);
    if(getQueueErrorCode(*queueToRemove) != 2)
    {
...
void *peek(Queue *queueToPeek)
{
    Node *tail = queueToPeek->dummy->next;
    //List is empty
    if(tail == NULL)
    {
        setQueueErrorCode(*queueToPeek ,2);
        return NULL;
    }
    //Get "head" of list
    Node* head = queueToPeek->dummy->next->next;
...
/* This function returns the number of objects 
   in the queue. */  
int getSize(Queue *queueToMeasure)
{
